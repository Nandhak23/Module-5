...
ALGORITHM
    ...
    Inside the Student class, define the __init__ method (constructor). The __init__ method accepts two parameters: name and student_id.
        Inside the __init__ method: Assign the value of name to self.name and student_id to self.student_id.
    Define the get_student_info method inside the Student class:
        This method should return a string formatted with self.name and self.student_id.
    Define the Attendance class, which inherits from the Student class.
    Inside the Attendance class, define the __init__ method (constructor).
    ...
    Define the check_eligibility method inside the Attendance class:
    ...
    Prompt the user to enter the name (as a string), student_id (as an integer), and attendance (as an integer).
    Create an instance student of the Attendance class, passing the entered name, student_id, and attendance to the constructor.
    Call the check_eligibility method on the student object and print the result.
